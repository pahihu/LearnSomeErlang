...
%% shell is itself a supervisor
%% a)
%%    {ok, Pid} = dropgen_sup:start_link().
%%    unlink(Pid).
%%
%% b)
%%    catch gen_server:call(dropgen, -60).
%%
%% c) turn off exception catch in shell
...
init([]) ->
   ...
   Drop = #{id       => 'dropgen',
            start    => {'dropgen', start_link, []},
            restart  => permanent,
            shutdown => 5000,
            type     => worker,
            modules  => ['dropgen']},        % dependencies
